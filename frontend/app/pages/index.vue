  ...
  <div class="flex flex-col gap-[125px]">
    <div class="relative overflow-hidden">
      ...
      >
        ...
        <template #title>
          <h1 class="text-4xl font-bold m-0 tracking-normal">
            Powerful biomodel simulations and tools <span class="text-blue-500">at your fingertips</span>
          </h1>
        </template>
        ...
        <div class="placeholder w-full h-[500px] bg-black rounded-lg" />
        <!--      <img
    ...
    <section
      id="marquees"
      class="w-full max-w-[1200px] px-6 mx-auto flex flex-col gap-[125px]"
    >
      <div class="w-full flex flex-col items-center gap-6">
        ...
        >
          ...
          >
            <img
              class="max-h-[40px] max-w-[250px] w-full h-full"
              :alt="partner.alt"
              :src="partner.src"
            >
          </NuxtLink>
        </UMarquee>
        <p class="text-sm opacity-50 text-center leading-7">
      ...
      <div class="w-full flex flex-col items-center gap-6">
        <p class="text-lg text-center">
          <strong>Built Using These Awesome Projects & Platforms</strong>
        </p>
        <UMarquee
          class="w-full overflow-hidden"
          pause-on-hover
        ...
        >
          <NuxtLink
            v-for="built_item of built_using"
            :key="built_item.url"
            :to="built_item.url"
            target="_blank"
          >
    ...
    <section
      id="revolutionary_platform"
      class="w-full relative px-6 py-[150px] text-white flex items-center text-center md:text-left justify-center"
    >
      <div
        id="revolutionary_content"
        class="w-full max-w-[1200px] mx-auto flex flex-col gap-2 z-4"
      >
        ...
        <div class="flex items-center gap-4 mt-3">
          <UButton
            class="w-max bg-transparent border-1 hover:bg-white/10 border-white text-white"
            variant="outline"
            color="neutral"
      ...
      />
      <div
        id="top_fade_gradient"
        class="absolute top-0 left-0 w-full h-full z-3"
      />
    </section>

    <section
      id="workflow"
      class="w-full px-6 text-center md:text-left"
    >
      <div
        id="workflow_content"
        class="w-full max-w-[1200px] mx-auto flex flex-col gap-8"
      >
        ...
        <div
          ref="workflow_container"
          class="w-full flex flex-col md:flex-col-reverse lg:flex-row items-center lg:items-stretch justify-center lg:justify-between gap-6"
        >
          <TransitionGroup
          ...
          >
            <div
              v-for="(workflow_feature, index) of workflow_features"
              :id="`workflow_feature_${index}`"
              :key="workflow_feature.title"
              tabindex="0"
              role="button"
              :aria-label="workflow_feature.title"
              :class="`workflow_feature py-1 cursor-pointer rounded-xl group flex flex-col gap-4 w-full ${workflow_feature.selected ? 'bg-gray-50 p-4!' : 'bg-transparent'}`"
              @keydown.enter.prevent="switch_workflow_feature(index)"
              @keydown.space.prevent="switch_workflow_feature(index)"
              @click="switch_workflow_feature(index)"
            >
              <div class="w-full flex flex-col">
                <p
                  class="text-lg"
                  :class="workflow_feature.selected ? 'text-black' : 'opacity-50 group-hover:opacity-100'"
                >
                  <strong>{{ workflow_feature.title }}</strong>
                </p>
                <p v-if="workflow_feature.selected">
                  {{ workflow_feature.description }}
                </p>
              </div>
              <!-- Static multimedia slot for mobile; renders the `associated_multimedia` of the current active WorkflowFeature -->
              <div
                v-if="workflow_feature.selected"
                id="mobile_multimedia_wrapper"
                class="w-full min-h-[100px] shadow-2xl overflow-hidden rounded-xl block md:hidden"
              >
                <img
                  v-if="current_workflow_feature && current_workflow_feature.associated_multimedia.type == 'image'"
                  class="object-cover"
                  :src="current_workflow_feature.associated_multimedia.src"
                  :alt="current_workflow_feature.associated_multimedia.alt"
                >
              </div>
              <div
                v-if="workflow_feature.selected"
                class="progress_wrapper w-full bg-gray-100 h-[6px] rounded-2xl relative overflow-hidden"
              >
                <div
                  class="progress_bar bg-blue-500 absolute w-0 h-full top-0 left-0 rounded-2xl group-hover:[--play-state:paused]"
                  :style="{ '--duration': `${workflow_feature.duration}ms` }"
                  @animationend="switch_workflow_feature()"
                />
              </div>
            </div>
          </TransitionGroup>
          ...
          <div
            id="multimedia_wrapper"
            class="w-full min-h-[100px] shadow-2xl overflow-hidden rounded-xl hidden md:block"
            data-aos="fade-up"
