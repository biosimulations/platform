...
let countdown_interval: ReturnType<typeof setInterval> | undefined = undefined
...
<template>
  <section class="w-full min-h-[calc(100vh-var(--ui-header-height))] px-6 max-w-[1200px] mx-auto flex flex-col gap-4 items-center justify-center text-center md:text-left">
    ...
    >
      ...
      />
      <h1 class="text-2xl font-bold md:whitespace-nowrap">
        Thank you for your submission
      </h1>
      <p v-if="!finished">
        Processing may take a while. We appreciate your patience.
      </p>
      <p v-if="finished">
        Your simulation(s) finished processing. View the result(s) below.
      </p>
      ...
      <section
        id="job_statuses"
        class="w-full grid items-stretch grid-cols-1 sm:grid-cols-2 sm:min-w-[600px] md:grid-cols-3 gap-4 p-4 md:w-max md:min-w-[700px] md:max-w-full lg:min-w-[900px] rounded-lg border border-gray-200 bg-white"
        :class="{ 'mt-4': finished }"
      >
        <div
          v-for="job of processing_status.jobs"
          :key="job.job_id"
          class="flex flex-col justify-between gap-4 flex-1 w-full border border-gray-200 bg-neutral-50/50 px-3 py-2 rounded-lg"
        >
          <div class="w-full flex flex-col gap-1">
            ...
            <div class="flex items-center gap-2 text-sm font-semibold pl-1">
              <UChip :color="job.status == 'processing' ? 'info' : (job.status == 'success' ? 'success' : 'error')" />
          ...
          >
            <UButton
              class="flex-1"
              type="button"
              :to="`https://biosimulations.org/runs/${job.biosimulations_run_id}`"
              target="_blank"
              variant="subtle"
              color="neutral"
              size="sm"
              icon="i-lucide-external-link"
              :label="job.status == 'success' ? 'See Result' : 'More Info'"
            />
            ...
            <UPopover
              v-if="job.status == 'failure'"
              class="flex-1"
              mode="hover"
              :ui="{ content: 'p-2 rounded bg-red-50 border border-red-200' }"
            >
              <UButton
                type="button"
                ...
                color="error"
                size="sm"
                icon="i-fluent-comment-error-24-regular"
                label="See Error"
              />
