...
const archive_file = ref<File | undefined>(undefined)
...
const simulator_version_options = ref<{ id: string, version: string }[]>([])
...
const form_invalid = computed(() => {
  ...
  if (stepper_position.value !== undefined) {
    ...
    const step_schema = schema.pick(
      // Dynamic field selection: current_fields holds runtime-known schema keys.
...
function advance() {
  if (steps.value[stepper_position.value]!.slot == 'simulators') {
    submission_payload.simulators = simulator_version_options.value.map((sim_version_option) => {
      return {
...
<template>
  <section class="w-full relative px-6 max-w-[1200px] mx-auto my-auto flex flex-col gap-4 items-center justify-center text-center md:text-left pt-5">
    ...
    <UForm
      :schema="schema"
      :state="(submission_payload as Record<string, unknown>)"
      @submit="submit()"
    >
      <UStepper
        ref="stepper"
        v-model="stepper_position"
        class="w-full md:w-[700px] mt-7"
        :items="steps"
      >
        <template #archive>
          <UCard
            class="w-full"
            :ui="{ body: 'bg-gray-50/50' }"
          >
            <div class="w-full sm:w-max mx-auto flex flex-col items-center gap-4">
              <UFileUpload
                v-model="archive_file"
                layout="list"
                accept=".omex"
                label="OMEX/Combine Archive"
                color="primary"
                description="Drop your OMEX/Combine archive here, or click to select"
                class="w-full sm:min-w-96 min-h-48"
                :class="{ 'opacity-50 pointer-events-none': archive_url, 'cursor-pointer': !archive_url }"
                @change="handle_file_change()"
              />
              <USeparator
                :class="!!archive_file || archive_url ? 'opacity-50' : 'opacity-100'"
                label="or"
              />
              <div
                class="w-full flex flex-col gap-1"
                :class="{ 'opacity-50': !!archive_file }"
              >
                <label for="omex_url"><small class="font-semibold">Enter URL for COMBINE/OMEX archive</small></label>
                <UInput
                  v-model="archive_url"
                  name="omex_url"
                  :disabled="!!archive_file"
        ...
        <template #simulators>
          ...
          >
            <h3 class="text-base font-bold">
              Select desired simulators and versions
            </h3>
            <div class="w-full mx-auto flex flex-col items-center gap-4 mt-2">
              <UFormField
                class="md:w-[250px]"
                label="Desired Simulators"
                name="simulators"
              >
                <USelectMenu
                  v-model="(submission_payload._simulators as never)"
                  class="w-full"
                  multiple
                  name="simulators_selection"
                  :items="(eligible_simulators as never)"
                  placeholder="Select desired simulator(s)"
                  label-key="name"
                />
              </UFormField>
              ...
              <UFormField
                v-for="(simulator, index) in submission_payload._simulators"
                :key="simulator.id"
                class="md:w-[250px]"
                :label="simulator.name + ' Version'"
                :name="`simulators.${index}._selected_version`"
              >
                <USelectMenu
                  v-model="simulator._selected_version"
                  class="w-full"
                  :disabled="!submission_payload._simulators || !submission_payload._simulators.length"
                  :items="simulator.versions"
        ...
        <template #end>
          ...
          >
            ...
            <div class="w-full mx-auto md:max-w-[250px] flex flex-col items-center gap-4 mt-2">
              ...
              >
                <UInput
                  v-model="submission_payload.name"
                  class="w-full"
              ...
              <div class="w-full flex flex-col gap-1">
                ...
                >
                  <UInput
                    v-model="submission_payload.email_address"
                    class="w-full"
                    type="email"
                ...
                <UTooltip
                  :delay-duration="0"
                  text="Providing your email address links you to the simulation, which would otherwise be submitted completely anonymously."
                >
                ...
                >
                  <UCheckbox
                    v-model="submission_payload.newsletter_consent"
              ...
              <UFormField
                label="Is this simulation for commercial or research/academic purposes?"
                name="is_commercial"
              >
                <URadioGroup
                  v-model="submission_payload.is_commercial"
                  required
                  :items="commercial_disclosure_options"
                />
              </UFormField>
              <!-- <div class="w-full flex flex-col gap-1">
      ...
      <div
        class="w-full md:max-w-full md:w-max md:min-w-[700px] flex gap-4 mt-4"
        :class="{ 'justify-between': stepper?.hasPrev, 'justify-end': !stepper?.hasPrev }"
      >
        <UButton
        ...
        />
        <UButton
          v-if="!archive_processed && stepper_position == 0"
          type="button"
          class="cursor-pointer"
          leading-icon="i-fluent-sparkle-20-filled"
          :disabled="!archive_url && !archive_file"
          :variant="!archive_url && !archive_file ? 'outline' : 'solid'"
          color="primary"
          :loading="processing_archive"
          :label="!archive_url && !archive_file ? 'Process File/Validate URL' : (archive_url ? (processing_archive ? 'Validating URL' : 'Validate URL') : (processing_archive ? 'Processing File' : 'Process File'))"
          @click="process_archive()"
        />
        <UButton
          v-if="archive_processed && stepper_position >= 0 && stepper_position < steps.length - 1"
          type="button"
          class="cursor-pointer"
          trailing-icon="i-lucide-arrow-right"
          :disabled="!stepper?.hasNext || !archive_processed || form_invalid"
          label="Next"
          @click="advance()"
        />
        <UButton
          v-if="archive_processed && stepper_position == steps.length - 1"
          type="submit"
          class="cursor-pointer"
          leading-icon="i-lucide-save"
          :disabled="form_invalid || submitting || !archive_processed"
          :loading="submitting"
